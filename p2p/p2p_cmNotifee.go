...
func (nn *cmNotifee) Connected(n network.Network, c network.Conn) {
	logger.Debugf("[Connected] %s (%s) dir: %d", c.RemotePeer().ShortString(), c.RemoteMultiaddr().String(), c.Stat().Direction)
}

func (nn *cmNotifee) Disconnected(n network.Network, c network.Conn) {
	logger.Debugf("[Disconnected] %s (%s) dir: %d", c.RemotePeer().ShortString(), c.RemoteMultiaddr().String(), c.Stat().Direction)
...
func (nn *cmNotifee) OpenedStream(n network.Network, s network.Stream) {
	logger.Debugf("[OpenedStream] %s (%s) memory: %p", s.Conn().RemotePeer().ShortString(), s.Protocol(), s)
}

func (nn *cmNotifee) ClosedStream(n network.Network, s network.Stream) {
	logger.Debugf("[ClosedStream] %s (%s) memory: %p", s.Conn().RemotePeer().ShortString(), s.Protocol(), s)
}
